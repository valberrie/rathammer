...
pub fn genMesh(sides: []const Side, alloc: std.mem.Allocator, map: *MeshMap) !void {
    const scale = 0.1;
    ...
    for (sides) |side| {
        ...
        const indexs = try triangulate(winding.items, alloc, @intCast(ret.mesh.vertices.items.len));
        const uvs = try calcUVCoords(winding.items, side, @intCast(ret.tex.w), @intCast(ret.tex.h), alloc);
        defer uvs.deinit();
        try ret.mesh.indicies.appendSlice(indexs.items);
        try ret.mesh.vertices.ensureUnusedCapacity(winding.items.len);
        for (winding.items, 0..) |vert, i| {
...
pub fn calcUVCoords(winding: []const Vec3, side: Side, tex_w: u32, tex_h: u32, alloc: std.mem.Allocator) !std.ArrayList(Vec2) {
    var uvs = try std.ArrayList(Vec2).initCapacity(alloc, winding.len);
